...
def populate_project_field(apps, schema_editor):
    ...
    """
    ProjectCostPerformance = apps.get_model('cost_performance', 'ProjectCostPerformance')
    Project = apps.get_model('projects', 'Project')

    # Get all unique project names from existing records
    unique_project_names = ProjectCostPerformance.objects.values_list(
    ...
    for record in ProjectCostPerformance.objects.filter(project__isnull=True):
...
def reverse_populate_project_field(apps, schema_editor):
    ...
    for record in ProjectCostPerformance.objects.filter(project__isnull=False):
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('cost_performance', '0002_alter_projectcostperformance_options_and_more'),
